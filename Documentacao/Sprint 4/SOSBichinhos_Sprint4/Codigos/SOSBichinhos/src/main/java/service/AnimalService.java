...
public class AnimalService {
    ...
    public Object update(Request request, Response response){
        ...
        if (animal != null){
            ...
            animal.setAdotado(registroAtualizado.getAdotado());
    ...
    public Object adotou(Request request, Response response){

        int id = Integer.parseInt(request.params(":id"));

        System.out.println("\n Id do animal adotado: " + id + " -> TRUE");

        Animal animal = (Animal) animalDAO.get(id);

        if (animal != null){
            animalDAO.adotado(id);  // Usando o objeto `animal` atualizado
            return id;
        } else{
            response.status(404); // 404 Not Found
            return "Animal não encontrado.";
    ...
    public Object voltou(Request request, Response response){

        int id = Integer.parseInt(request.params(":id"));

        System.out.println("\n Id do animal adotado: " + id + " -> FALSE");

        Animal animal = (Animal) animalDAO.get(id);

        if (animal != null){
            animalDAO.voltou(id);  // Usando o objeto `animal` atualizado
            return id;
        } else{
            response.status(404); // 404 Not Found
            return "Animal não encontrado.";
