document.addEventListener("DOMContentLoaded", () => {
	...
	// Função para buscar os detalhes do endereço da pessoa
	async function getEnderecoDetails() {
		...
		try {
			...
			enderecos = await response.json(); // Faz o parsing da resposta 
			return enderecos;
		} catch (error) {
			console.error('Erro:', error);
		}
	...
	SubmeterFormBtn.addEventListener("click", async (event) => {
		...
		try {
			...
			document.getElementById("formularioAdocao").reset(); // Limpa os campos após a submissão
		} catch (error) {
			console.error("Erro ao enviar o formulário:", error);
		}
	});

	//Botão de Listar Formulários do Usuário
	...
	// Função para abrir o modal de animais adotados
	openModalAnimaisBtn.onclick = async function() {
		...
		try {
			...
			ListarAnimais(formularios); // Chama a função de listar animais adotados
		} catch (error) {
			console.error('Erro ao carregar formulários:', error);
		}
	};

	// Função para abrir o modal de formulários submetidos
	openModalFormulariosBtn.onclick = async function() {
		...
		try {
			const response = await fetch(apiUrl_Formulario); 
			const formularios = await response.json();
			Listar(formularios); // Chama a função de listar formulários submetidos
		} catch (error) {
			console.error('Erro ao carregar formulários:', error);
		}
	};

	//Função para Listar Animais adotados do Usuário Logado
	async function ListarAnimais(formularios) {
		...
		modalBody.innerHTML += animaisHTML.join(''); // Adiciona todos os animais ao modal de uma vez
	}

	//Função para Listar formulários submetidos pelo usuario logado
	function Listar(formularios) {
		...
		//De boolean ara string
		function booleanToString(value) {
			return value ? "Sim" : "Não";
		}
		
		formulariosDoUsuario.forEach((formulario) => {
			// Exibe detalhes de cada formulário no modal
			modalBody.innerHTML += `
			<div class="flexPerfilJP">
				<div class="informacoesJP">
